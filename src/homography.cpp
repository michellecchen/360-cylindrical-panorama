...
vector<vector<float>> harris(const FloatImage im, int levels, vector<FloatImage> pyramid){
    ...
    for(int l = 0; l < levels; l++){
        ...
        FloatImage corners(harrisPyra[l][0].width(), harrisPyra[l][0].height(), harrisPyra[l][0].channels());

        // calculate and store corner values
        for(int y = 0; y < harrisPyra[l][0].height(); y++){
            for(int x = 0; x < harrisPyra[l][0].width(); x++){
                ...
                corners(x,y,0) = det - 0.04*pow(trace, 2);
            }
        }
        /////////////////////////////////////
